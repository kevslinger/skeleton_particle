...
def do_stuff(directory):
    ...
    for i in range(len(glob.glob(os.path.join(particle_path, '*.csv')))):
        ...
        skeleton.drop(range(1, len(skeleton)), inplace=True)
        skele_df = pd.concat([skele_df, skeleton], ignore_index=True, sort=False)
        ...
        particle.drop(range(1, len(particle)), inplace=True)
