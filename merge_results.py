...
def do_stuff(directory):
    ...
    for i in range(len(glob.glob(os.path.join(particle_path, '*.csv')))):
        ...
        skele_df = pd.concat([skele_df, skeleton[skeleton['# Branches'] == max(skeleton['# Branches'])]], ignore_index=True, sort=True)
        ...
        particle_df = pd.concat([particle_df, particle[particle['Area'] == max(particle['Area'])]], ignore_index=True, sort=True)
